<div class="container mx-auto px-4 sm:px-6 lg:px-8">
  <x-title title="Update Customer Information" :backUrl="route('customers')" />
  <div class="mt-0 mx-auto">
    <form wire:submit="save">
      <div class="px-4 py-5 bg-white sm:p-6 shadow">
        <div class="grid grid-cols-1 gap-3">
          <div>
            <x-filepond::upload wire:model="profile_photo" class="w-[15%]" allowImagePreview stylePanelLayout=compact
              stylePanelAspectRatio=1 allowMultiple="false" />
            <x-input-error for="profile_photo" />
          </div>
          <!-- status -->
          <div class="mt-2">
            <x-label for="status" value="Status" />
            <x-select-input wire:model="status" :options="$statuses" selected="new" placeholder="Status" />
